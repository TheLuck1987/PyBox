...
{
    public class SimpleDataService : IScriptDataService
    {
        ...
        {
            if (input.Title == null)
                return new ScriptDataServiceResponse() { ErrorLevel = Enums.WarningLevel.ERROR, Errors = "Title is empty", Result = null };
            IEnumerable<ScriptEntity> query = _database.Where(d => d.DeletedAt == null && d.Title!.ToUpper().Trim() == input.Title!.ToUpper().Trim());
            if (input.Id != null)
                query = query.Where(q => q.ScriptId != input.Id);
